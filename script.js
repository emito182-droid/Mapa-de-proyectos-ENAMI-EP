// 1. Detección de Datos (GitHub o Local)
...
// 2. Función para GitHub
function copyForGitHub() {
    const code = `const DATA_CENTRAL = ${JSON.stringify(projects, null, 4)};`;
    navigator.clipboard.writeText(code).then(() => {
        alert("CÓDIGO COPIADO. Ve a GitHub, abre 'proyectos.js', borra todo y pega este contenido.");
    });
}
...
function showProjectDetails(proj) {
    ...
    let html = `
        <div class="mb-6 border-b pb-4 flex justify-between items-center">
            ...
            <button onclick="editProject(${proj.id})" class="bg-yellow-500 text-white p-2 rounded shadow-sm">✎</button>
        </div>
        <div class="space-y-6">`;
...
function editProject(id) {
    ...
    proj.concesiones.forEach(c => {
        ...
        lb.querySelector('.c-sup').value = c.superficie;
...
function addConcesionForm() {
    ...
    div.innerHTML = `
        ...
        <div class="grid grid-cols-2 gap-4 mb-4"><input type="text" class="c-nombre border-b p-2 text-xs font-bold uppercase" placeholder="CONCESIÓN" required><input type="text" class="c-codigo border-b p-2 text-xs" placeholder="CÓDIGO"></div>
        <div class="grid grid-cols-3 gap-2 mb-4"><input type="text" class="c-prov border p-2 text-[9px] uppercase" placeholder="PROVINCIA"><input type="text" class="c-cant border p-2 text-[9px] uppercase" placeholder="CANTÓN"><input type="text" class="c-parr border p-2 text-[9px] uppercase" placeholder="PARROQUIA"></div>
        <div class="grid grid-cols-2 gap-4 mb-4"><select class="c-tipo border p-1"><option value="METÁLICA">METÁLICA</option><option value="NO METÁLICA">NO METÁLICA</option></select><input type="number" class="c-sup border p-1" placeholder="HA"></div>
        <div class="grid grid-cols-2 gap-4 mb-4"><select class="c-reg border p-1 text-[9px]"><option value="GRAN MINERÍA">GRAN MINERÍA</option><option value="RÉGIMEN GENERAL">RÉGIMEN GENERAL</option><option value="PEQUEÑA MINERÍA">PEQUEÑA MINERÍA</option></select><select class="c-per border p-1 text-[9px]"><option value="EXPLORACIÓN INICIAL">EXPLORACIÓN INICIAL</option><option value="EXPLORACIÓN AVANZADA">EXPLORACIÓN AVANZADA</option><option value="EXPLORACIÓN-EXPLOTACIÓN">EXPLORACIÓN-EXPLOTACIÓN</option></select></div>
...
document.getElementById('mainForm').addEventListener('submit', (e) => {
    ...
    alert("Guardado Local. Copia el código para GitHub.");
    location.reload();
});
...
async function generatePDF(pId, cod) {
    ...
    doc.autoTable({ startY: 50, body: [['PROYECTO', pr.nombreMaestro],['CONCESIÓN', c.nombre],['UBICACIÓN', c.provincia],['HA', c.superficie]], theme: 'striped' });
