...
// FUNCIÓN NUEVA: Copiar código listo para proyectos.js
function copyForGitHub() {
    const code = `const DATA_CENTRAL = ${JSON.stringify(projects, null, 4)};`;
    navigator.clipboard.writeText(code).then(() => {
        alert("CÓDIGO COPIADO. Ahora ve a GitHub, abre 'proyectos.js', borra todo y pega este contenido.");
    });
}
...
// 3. ACTUALIZACIÓN DEL PANEL LATERAL
...
function renderMarkers() {
    ...
    projects.forEach(proj => {
        if (proj.concesiones && proj.concesiones.length > 0) {
            ...
            m.bindTooltip(proj.nombreMaestro, { permanent: true, direction: 'top', offset: [0, -10], className: 'font-bold uppercase shadow-md' }).openTooltip();
...
document.getElementById('mainForm').addEventListener('submit', (e) => {
    ...
    alert("Guardado Localmente. Use el botón superior para copiar el código a GitHub.");
    location.reload();
});
...
async function generatePDF(projId, cod) {
    ...
    doc.text('ENAMI EP - REPORTE TÉCNICO', 14, 25);
    doc.autoTable({
        startY: 50,
        ...
        body: [
            ...
            ['Concesión', c.nombre],
            ['Código', c.codigo],
            ['Ubicación', `${c.provincia} - ${c.canton}`],
            ['Superficie', c.superficie + ' Ha'],
            ['Tipo', c.tipo],
